...
.
.TH "COMPUTE" "1" "Dec 20, 2024" "3.51.1" "OCI CLI Command Reference"
.SH NAME
compute \- 
.
...
.UNINDENT
.IP \(bu 2
instance\-maintenance\-reboot
.INDENT 2.0
.IP \(bu 2
get
.UNINDENT
.IP \(bu 2
measured\-boot\-report
.INDENT 2.0
.IP \(bu 2
...
.UNINDENT
.UNINDENT
.SS global\-image\-capability\-schema
.SS Description
.sp
Global Image Capability Schema
.SS Available Commands
...
.UNINDENT
.UNINDENT
.SS global\-image\-capability\-schema\-version
.SS Description
.sp
Global Image Capability Schema Version
.SS Available Commands
...
.UNINDENT
.UNINDENT
.SS image\-shape\-compatibility\-entry
.SS Description
.sp
...
.sp
For more information, see \fI\%Overview of the Compute Service\fP <\fBhttps://docs.cloud.oracle.com/iaas/Content/Compute/Concepts/computeoverview.htm\fP>\&.
.sp
To use any of the API operations, you must be authorized in an IAM policy. If you’re not authorized, talk to an administrator. If you’re an administrator who needs to write policies to give users access, see \fI\%Getting Started with Policies\fP <\fBhttps://docs.cloud.oracle.com/iaas/Content/Identity/Concepts/policygetstarted.htm\fP>\&.
.sp
\fBWarning:\fP Oracle recommends that you avoid using any confidential information when you supply string values using the API.
...
.INDENT 0.0
.IP \(bu 2
\fBSTART\fP \- Powers on the instance.
.IP \(bu 2
\fBSTOP\fP \- Powers off the instance.
.IP \(bu 2
\fBRESET\fP \- Powers off the instance and then powers it back on.
.IP \(bu 2
\fBSOFTSTOP\fP \- Gracefully shuts down the instance by sending a shutdown command to the operating system. After waiting 15 minutes for the OS to shut down, the instance is powered off. If the applications that run on the instance take more than 15 minutes to shut down, they could be improperly stopped, resulting in data corruption. To avoid this, manually shut down the instance using the commands available in the OS before you softstop the instance.
.IP \(bu 2
\fBSOFTRESET\fP \- Gracefully reboots the instance by sending a shutdown command to the operating system. After waiting 15 minutes for the OS to shut down, the instance is powered off and then powered back on.
.IP \(bu 2
\fBSENDDIAGNOSTICINTERRUPT\fP \- For advanced users. \fBWarning: Sending a diagnostic interrupt to a live system can cause data corruption or system failure.\fP Sends a diagnostic interrupt that causes the instance’s OS to crash and then reboot. Before you send a diagnostic interrupt, you must configure the instance to generate a crash dump file when it crashes. The crash dump captures information about the state of the OS at the time of the crash. After the OS restarts, you can analyze the crash dump to diagnose the issue. For more information, see \fI\%Sending a Diagnostic Interrupt\fP <\fBhttps://docs.cloud.oracle.com/iaas/Content/Compute/Tasks/sendingdiagnosticinterrupt.htm\fP>\&.
.UNINDENT
.INDENT 0.0
.INDENT 3.5
.INDENT 0.0
.IP \(bu 2
\fBDIAGNOSTICREBOOT\fP \- Powers off the instance, rebuilds it on the physical host, and then powers it back on. Before you send a diagnostic reboot, restart the instance’s OS, confirm that the instance and networking settings are configured correctly, and try other \fI\%troubleshooting steps\fP <\fBhttps://docs.cloud.oracle.com/iaas/Content/Compute/References/troubleshooting-compute-instances.htm\fP>\&. Use diagnostic reboot as a final attempt to troubleshoot an unreachable instance. For virtual machine (VM) instances only. For more information, see \fI\%Performing a Diagnostic Reboot\fP <\fBhttps://docs.cloud.oracle.com/iaas/Content/Compute/Tasks/diagnostic-reboot.htm\fP>\&.
.IP \(bu 2
\fBREBOOTMIGRATE\fP \- Powers off the instance, moves it to new hardware, and then powers it back on.
.UNINDENT
.sp
For more information about managing instance lifecycle states, see \fI\%Stopping and Starting an Instance\fP <\fBhttps://docs.cloud.oracle.com/iaas/Content/Compute/Tasks/restartinginstance.htm\fP>\&.
.UNINDENT
.UNINDENT
.SS Usage
.INDENT 0.0
...
.INDENT 0.0
.TP
.B \-\-allow\-dense\-reboot\-migration [boolean]
.UNINDENT
.sp
For use with RESET and SOFTRESET actions on instances that use a DenseIO shape, the flag denoting whether \fI\%reboot migration\fP <\fBhttps://docs.cloud.oracle.com/iaas/Content/Compute/References/infrastructure-maintenance.htm#reboot\fP> is performed for the instance. The default value is \fIfalse\fP\&.
.sp
If the instance has a date in the Maintenance reboot field and you do nothing (or set this flag to \fIfalse\fP), the instance will be rebuilt at the scheduled maintenance time. The instance will experience 2\-6 hours of downtime during the maintenance process. The local NVMe\-based SSD will be preserved.
.sp
If you want to minimize downtime and can delete the SSD, you can set this flag to \fItrue\fP and proactively reboot the instance before the scheduled maintenance time. The instance will be reboot migrated to a healthy host and the SSD will be deleted. A short downtime occurs during the migration.
.sp
\fBCaution:\fP When \fItrue\fP, the SSD is permanently deleted. We recommend that you create a backup of the SSD before proceeding.
.INDENT 0.0
.TP
.B \-\-delete\-local\-storage [boolean]
.UNINDENT
.sp
...
.UNINDENT
.sp
If present, this parameter will set (or re\-set) the scheduled time that the instance will be reboot migrated in the format defined by \fI\%RFC3339\fP <\fBhttps://tools.ietf.org/html/rfc3339\fP>\&.  This will also change the timeRebootMigrationDue field on the instance. If not present, the reboot migration will be triggered immediately.
.INDENT 0.0
.INDENT 3.5
...
.UNINDENT
.UNINDENT
.SS \fBget\-windows\-initial\-creds\fP
.INDENT 0.0
.IP \(bu 2
\fI\%Description\fP
.IP \(bu 2
...
.sp
When launching an instance, you may provide the \fIsecurityAttributes\fP parameter in \fI\%LaunchInstanceDetails\fP <\fBhttps://docs.cloud.oracle.com/api/#/en/iaas/latest/LaunchInstanceDetails\fP> to manage security attributes via the instance, or in the embedded \fI\%CreateVnicDetails\fP <\fBhttps://docs.cloud.oracle.com/api/#/en/iaas/latest/CreateVnicDetails/\fP> to manage security attributes via the VNIC directly, but not both.  Providing \fIsecurityAttributes\fP in both locations will return a 400 Bad Request response.
.sp
...
.sp
This option is a JSON list with items of type LaunchInstanceLicensingConfig.  For documentation on LaunchInstanceLicensingConfig please see our API reference: \fI\%https://docs.cloud.oracle.com/api/#/en/iaas/20160918/datatypes/LaunchInstanceLicensingConfig\fP\&.
This is a complex type whose value must be valid JSON. The value can be provided as a string on the command line or passed in as a file using
the \fI\%file://path/to/file\fP syntax.
.sp
The \fB\-\-generate\-param\-json\-input\fP option can be used to generate an example of the JSON which must be provided. We recommend storing this example
...
.UNINDENT
.UNINDENT
.SS \fBupdate\-instance\-amd\-vm\-update\-instance\-platform\-config\fP
.INDENT 0.0
.IP \(bu 2
\fI\%Description\fP
.IP \(bu 2
...
.UNINDENT
.UNINDENT
.SS \fBupdate\-instance\-intel\-vm\-update\-instance\-platform\-config\fP
.INDENT 0.0
.IP \(bu 2
\fI\%Description\fP
.IP \(bu 2
...
.UNINDENT
.UNINDENT
.SS \fBupdate\-instance\-update\-instance\-source\-via\-boot\-volume\-details\fP
.INDENT 0.0
.IP \(bu 2
\fI\%Description\fP
.IP \(bu 2
...
.UNINDENT
.UNINDENT
.SS \fBupdate\-instance\-update\-instance\-source\-via\-image\-details\fP
.INDENT 0.0
.IP \(bu 2
\fI\%Description\fP
.IP \(bu 2
...
.UNINDENT
.UNINDENT
.SS instance\-console\-connection
.SS Description
.sp
...
.UNINDENT
.UNINDENT
.SS instance\-maintenance\-reboot
.SS Description
.sp
